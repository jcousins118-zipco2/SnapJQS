/**
 * juried-layer/governor.ts
 * 
 * GOVERNOR LAYER
 * 
 * Enforces that model output is always a full continuable artifact —
 * never a bare INVALID / refusal / one-liner.
 * 
 * Key exports:
 *   governorAccepts()           — validates output is a well-formed 7-step artifact
 *   applyObligationLint()       — pre-jury prompt wrapper that prevents poison obligations
 *   forceFullArtifactRepairPrompt() — retry prompt when first attempt is rejected
 *   fallbackFullPlanArtifact()  — deterministic 7-step shell (last resort)
 *   stripBoilerplateIfPresent() — strips Framing/Design-Space preamble from output
 *   applyGovernorToPrompt()     — legacy alias for applyObligationLint
 */
...
/** Returns true iff output is a full 7-step artifact with required fields. */
...
/**
 * Pre-jury prompt wrapper.
 * Prevents poison obligations (global/absolute promises) from entering the model.
 */
...
/** Repair prompt used when first model attempt fails governor check. */
...
/** Deterministic fallback: always produces a 7-step shell satisfying the Governor contract. */
...
/** Strips Framing / Design Space / Example Plan preamble — keeps from Step 1 onward. */
...
/** Legacy alias: wraps user prompt with governor obligation lint. */
